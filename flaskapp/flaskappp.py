...
def api_recommender():
	if 'classname' in request.args:
		userReccomend=request.args.get('classname')
	else:
		return 'Error Recommender'
	# userReccomend
	print userReccomend
	os.chdir("advertisement")
#	paTh="/home/scorpio/Smart-Intelligent-User-Profiling-E-Commerce/SmartUserProfilingML/images/"
	googlepath="https://storage.googleapis.com/imagees/"
#	newpath=paTh+userReccomend
	newgooglepath=googlepath+userReccomend

	line = random.choice(open(userReccomend).readlines())
	line=line[:-1]
	print line  #images
#	os.chdir(newpath)##path for images
#	print "search images"
	#Imagepath=newpath+"/"+line
	googleImage=newgooglepath+"/"+line
	os.chdir("/home/scorpio/flaskapp/")
	cwd=os.getcwd()
	print cwd

	js = json.dumps(googleImage)
	resp = Response(js, status=200, mimetype='application/json')
	return resp
